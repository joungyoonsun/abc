...
public class JHBoardService implements IJHBoardService{
	...
	public void insertJHBoardArticle(JHBoard jhBoard) {
		jhBoard.setJhId(jhBoardRepository.selectMaxJHBoardArticleNo()+1);
		jhBoardRepository.insertJHBoardArticle(jhBoard);
	}

	@Transactional
	public void insertJHBoardArticle(JHBoard jhBoard, JHBoardUploadFile jhFile) {
		jhBoard.setJhId(jhBoardRepository.selectMaxJHBoardArticleNo()+1);
		jhBoardRepository.insertJHBoardArticle(jhBoard);
		if(jhFile != null && jhFile.getJhFileName() != null && !jhFile.getJhFileName().equals("")) {
			//���ϸ��� ���ų� ������ ���� ���ԺҰ�
			jhFile.setJhId(jhBoard.getJhId());
			jhFile.setJhFileId(jhBoardRepository.selectMaxJHBoardFileId()+1);
	...
	@Transactional
	public JHBoard selectJHBoardArticle(int jhId) {
		jhBoardRepository.updateJHBoardReadCount(jhId);
		return jhBoardRepository.selectJHBoardArticle(jhId);
	}
