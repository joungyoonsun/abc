...
public class EmpBoardService implements IEmpBoardService {
	...
	public void insertEmpBoardArticle(EmpBoard empId) {
		empId.setEmpId(empBoardRepository.selectMaxEmpBoardArticleNo()+1);
	...
	public void insertEmpBoardArticle(EmpBoard empId, EmpUploadFile file) {
		empId.setEmpId(empBoardRepository.selectMaxEmpBoardArticleNo()+1);
		empBoardRepository.insertEmpBoardArticle(empId);
		if(file != null && file.getEmpFileName() != null && !file.getEmpFileName().equals("")) {
			file.setEmpId(empId.getEmpId());
			file.setEmpFileId(empBoardRepository.selectMaxEmpBoardFileId()+1);
	...
	//게시글 상세 조회
	@Transactional
	public EmpBoard selectEmpBoardArticle(int empId) {
		empBoardRepository.updateEmpBoardReadCount(empId);
		return empBoardRepository.selectEmpBoardArticle(empId);
	}
	//게시글 수정
	@Override
